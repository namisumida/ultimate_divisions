...
metrics_df$successful_throw <- 1 # assume all successes
# remove some irrelevant ones, like 'score', 'turn', 'time-out'
metrics_df$successful_throw <- ifelse(metrics_df$throw=='score'|metrics_df$throw=='turn'|metrics_df$throw=='time-out', 0, metrics_df$successful_throw)
i <- 1
for (i in 1:nrow(metrics_df)) { # for every row in metrics_df
  if (metrics_df$throw[i]=='turn') { # if it's a turn
    if (metrics_df$throw[i-1]!='score' & metrics_df$throw[i-1]!='turn' & metrics_df$throw[i-1]!='time-out' & metrics_df$throw[i-1]!='turn') {
...
create_possessions <- function(game_id_input, team_type_input) {
  ...
  while (i<=nrow(metrics_subset)) { # while there are still more rows in metrics df
    ...
    while (metrics_subset$throw[i]=='over' | metrics_subset$throw[i]=='dish' | metrics_subset$throw[i]=='down' | metrics_subset$throw[i]=='dump' | 
      ...
      if (metrics_subset$throw[i]!='time-out' & metrics_subset$throw[i]!='stalled') {
        n_throws <- n_throws + 1 # count number of throws
        break_throws <- break_throws + metrics_subset$break_throw[i]
      }
    ...
    possessions$break_throw[j] <- break_throws # final count of break throws in possession
...
colnames(possessions) <- c('game_id', 'team_name', 'team_type', 'point', 'possession_result', 'sum_breakthrows', 'starting_fieldpos', 'defense', 'division', 'successful_throw', 'sum_throws', 'last_throw', 'first_throw')
rownames(possessions) <- seq(length=nrow(possessions)) # reset row index
possessions$possession_result <- ifelse(possessions$possession_result=='callahan', 'score', possessions$possession_result)

# Create df from possessions grouped by points
points_grp <- possessions %>% group_by(game_id, point, team_type) %>%
  summarise(n_possessions = n(), n_throws = sum(sum_throws), n_breakthrows = sum(sum_breakthrows), 
            n_turns = sum(possession_result=='turn'), scored = sum(possession_result=='score'))
points_merged <- merge(points_grp, points, by = c('game_id', 'point'))
points_merged$starting_o <- ifelse(points_merged$start_o==points_merged$team_type, 1, 0)
points_merged <- points_merged[,-10]
# diagnostics
...
o_points <- subset(points_merged, starting_o==1)
d_points <- subset(points_merged, starting_o==0) # fewer D points because of the no turnover O points. If O doesn't turn it over, D never gets to have possession

# Proportion of throws in each game
throws_sub <- subset(person_metrics, throw!='callahan' & throw!='turn' & throw!='time-out' & throw!='score' & throw!='dish' & throw!="stalled")
prop.table(table(throws_sub$throw, throws_sub$division), 2)*100 # throws by game

# success rate of certain throws
prop.table(table(subset(person_metrics, division=="womens")$successful_throw))*100
prop.table(table(subset(person_metrics, division=="mixed")$successful_throw))*100
prop.table(table(subset(person_metrics, division=="mens")$successful_throw))*100
...
nrow(subset(turns_points, division=="womens" & n_turns>0))/nrow(subset(points_merged, division=="womens")) 
nrow(subset(turns_points, division=="mixed" & n_turns>0))/nrow(subset(points_merged, division=="mixed")) 
nrow(subset(turns_points, division=="mens" & n_turns>0))/nrow(subset(points_merged, division=="mens")) 
...
prop.table(table(person_possessions$starting_fieldpos))*100 # all points
post_turnover_sub <- subset(person_possessions, possession_number>1 | (possession_number==1 & start_o==0))
table(subset(person_possessions, possession_number==1 & start_o!=0)$starting_fieldpos) # check
prop.table(table(post_turnover_sub$starting_fieldpos, post_turnover_sub$division),2)*100 # after turnovers


### OFFENSIVE EFFICIENCY
# Offensive holds
prop.table(table(subset(o_points, division=='womens' & team_type=='win_team')$scored))*100 # winning team
prop.table(table(subset(o_points, division=='mixed' & team_type=='win_team')$scored))*100 # winning team
prop.table(table(subset(o_points, division=='mens' & team_type=='win_team')$scored))*100 # winning team

# Percent of points that had no turnovers - winning teams
nrow(subset(o_points, division=="womens" & n_turns==0 & team_type=="win_team"))/nrow(subset(o_points, division=="womens" & team_type=="win_team"))*100 # for winning teams' O line
nrow(subset(o_points, division=="mixed" & n_turns==0 & team_type=="win_team"))/nrow(subset(o_points, division=="mixed" & team_type=="win_team"))*100 # for winning teams' O line
nrow(subset(o_points, division=="mens" & n_turns==0 & team_type=="win_team"))/nrow(subset(o_points, division=="mens" & team_type=="win_team"))*100 # for winning teams' O line

# Rebased to be out of holds 
nrow(subset(o_points, division=="womens" & n_turns==0 & starting_o==1 & team_type=="win_team"))/nrow(subset(o_points, division=="womens" & team_type=="win_team" & scored==1))*100 # for winning teams' O line
nrow(subset(o_points, division=="mixed" & n_turns==0 & starting_o==1  & team_type=="win_team"))/nrow(subset(o_points, division=="mixed" & team_type=="win_team" & scored==1))*100 # for winning teams' O line
nrow(subset(o_points, division=="mens" & n_turns==0 & starting_o==1 & team_type=="win_team"))/nrow(subset(o_points, division=="mens" & team_type=="win_team" & scored==1))*100 # for winning teams' O line

# Percent of points that had no turnovers out of holds - losing teams
nrow(subset(o_points, division=="womens" & n_turns==0 & starting_o==1 & team_type=="lose_team"))/nrow(subset(o_points, division=="womens" & team_type=="lose_team" & scored==1))*100 # for winning teams' O line
nrow(subset(o_points, division=="mixed" & n_turns==0 & starting_o==1  & team_type=="lose_team"))/nrow(subset(o_points, division=="mixed" & team_type=="lose_team" & scored==1))*100 # for winning teams' O line
nrow(subset(o_points, division=="mens" & n_turns==0 & starting_o==1 & team_type=="lose_team"))/nrow(subset(o_points, division=="mens" & team_type=="lose_team" & scored==1))*100 # for winning teams' O line


### DEFENSE
# Successful defensive rate - winnin teams
# D-line points that generated turns
nrow(subset(points_merged, division=="womens" & team_type=='win_team' & starting_o==0))/nrow(subset(points, division=="womens" & start_o=='lose_team'))*100
nrow(subset(points_merged, division=="mixed" & team_type=='win_team' & starting_o==0))/nrow(subset(points, division=="mixed" & start_o=='lose_team'))*100
nrow(subset(points_merged, division=="mens" & team_type=='win_team' & starting_o==0))/nrow(subset(points, division=="mens" & start_o=='lose_team'))*100

# D-line scores out of all had-disc points
nrow(subset(points_merged, division=="womens" & team_type=='win_team' & start_o=="lose_team" & scored==1))/nrow(subset(points_merged, division=="womens" & start_o=='lose_team' & team_type=="win_team" & n_possessions>0)) 
nrow(subset(points_merged, division=="mixed" & team_type=='win_team' & starting_o==0 & scored==1))/nrow(subset(points_merged, division=="mixed" & start_o=='lose_team' & team_type=="win_team" & n_possessions>0)) 
nrow(subset(points_merged, division=="mens" & team_type=='win_team' & starting_o==0 & scored==1))/nrow(subset(points_merged, division=="mens" & start_o=='lose_team' & team_type=="win_team" & n_possessions>0)) 
